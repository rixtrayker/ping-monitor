...
class Monitor extends EventEmitter {
  ...
  updateAvgResponseTime(responseTime) {
    const totalCompletedRequests = this.totalRequests - this.totalDownTimes - 1;
    const sumOfResponseTimes = this.avgResponseTime * totalCompletedRequests + responseTime;
    this.avgResponseTime = sumOfResponseTimes / (totalCompletedRequests + 1);
  }
  ...
  respond(statusCode, data, error) {
    ...
    if (isUp) {
      if(!this.isUp) {
        ...
        this.updateAvgResponseTime(responseData.responseTime);
      }
  ...
  updateAvgResponseTime(responseTime) {
    const totalCompletedRequests = this.totalRequests - this.totalDownTimes;
    const sumOfResponseTimes = this.avgResponseTime * totalCompletedRequests + responseTime;
    this.avgResponseTime = sumOfResponseTimes / (totalCompletedRequests + 1);
  }
